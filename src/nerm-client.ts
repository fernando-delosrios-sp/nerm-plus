...
export class NERMClient {
    ...
    async resolveAttributePath(profile: any, path: string): Promise<{ profile: any; path: string }> {
        // ⚡ Bolt: Replace O(N) array allocation chain (split, reverse, join) with O(1) string slicing
        // Impact: Reduces memory overhead and GC pressure during recursive path resolution
