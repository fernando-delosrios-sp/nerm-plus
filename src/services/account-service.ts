        ...
        switch (type) {
            case 'Profile': {
                ...
                const relevantAttrs = schema!.attributes.filter((attr) => {
                    ...
                    // ⚡ Bolt: Use allocation-free string slicing instead of split().pop()
                    const lastIndex = attr.name.lastIndexOf('.')
                    const leaf = lastIndex === -1 ? attr.name : attr.name.slice(lastIndex + 1)
                ...
                const attrResults = (
                    await Promise.all(
                        relevantAttrs.map(async (attribute) => {
                            // ⚡ Bolt: Use allocation-free string slicing instead of split().pop()
                            const lastIndex = attribute.name.lastIndexOf('.')
                            const leaf = lastIndex === -1 ? attribute.name : attribute.name.slice(lastIndex + 1)
                            const rawValue =
