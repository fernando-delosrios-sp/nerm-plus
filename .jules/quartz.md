...
## 2026-06-30 - Replace finally with direct returns in try/catch

**Learning:** Returning from a `finally` block suppresses any errors thrown in the `catch` block and requires maintaining mutable state, which adds unnecessary cognitive load and hides potential failures.
**Action:** Replace mutable state and `finally` block returns (e.g., `finally { return item; }`) with direct `return` statements inside `try` and `catch` blocks to improve scanability, simplify control flow, and prevent suppressing thrown errors.
