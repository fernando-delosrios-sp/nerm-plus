...
## 2026-08-18 - Replace finally return blocks with direct try/catch returns
**Learning:** Using mutable variables and returning them inside a `finally` block creates unnecessary intermediate state, makes control flow harder to track, and can unexpectedly swallow thrown errors because `finally` returns override them.
**Action:** Replace `finally { return state; }` blocks with direct `return` statements inside both the `try` and `catch` blocks. This eliminates mutable variables and clarifies the exact fallback behavior.
