...
## 2026-06-10 - Pass strictly typed union variables directly instead of branching
**Learning:** When variables are strictly typed to a specific union (e.g., `'NeprofileUser' | 'NeaccessUser'` via `getRoleType`), they are often unwrapped through redundant `if-else` blocks that check for one value and fall back to another before passing to a function.
**Action:** Pass the variable directly to consuming functions rather than unwrapping it through redundant `if-else` blocks. This reduces indentation and visual noise.
