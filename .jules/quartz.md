...
## 2024-05-19 - Explicitly test falsy values vs null

**Learning:** When writing unit tests for attribute mappers or functions that utilize the Nullish Coalescing Operator (`??`), it is easy to assume it works. Testing explicit falsy values (like `0`, `false`, and `""`) alongside `null` confirms they are correctly preserved.
**Action:** In future test additions targeting property mappers, ensure explicit assertions exist for `null` inputs as well as falsy inputs like `0` or `false` to prevent regressions if developers later change `??` to `||`.
