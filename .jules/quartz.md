...
## 2024-06-20 - Simplify undefined/null checks with loose inequality and nullish coalescing
**Learning:** Verbose, multi-part strict checks like `x !== undefined && x !== null` are a common pattern in the codebase, often combined with ternary fallbacks `? x : y`. These add visual noise and increase cognitive load.
**Action:** Consistently replace `x !== undefined && x !== null` with the functionally identical loose inequality `x != null`. Furthermore, when this is used in a fallback ternary, replace the entire expression with the nullish coalescing operator `x ?? y` to improve readability while strictly preserving falsy values.
