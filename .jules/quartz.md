...
## 2026-06-17 - Simplify nullish checks and fallback assignments
**Learning:** Verbose checks like `!== undefined && !== null` combined with ternaries for fallback assignments add unnecessary visual noise and cognitive load.
**Action:** Use loose inequality (`!= null`) to catch both null and undefined, and use the nullish coalescing operator (`??`) for clean fallback assignments to dramatically improve readability and intent.
