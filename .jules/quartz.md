...
## 2026-06-13 - Simplify nullish checks with operators
**Learning:** Verbose checks like `!== undefined && !== null` and conditional fallbacks add visual noise and make the intent of handling nullable values harder to quickly scan.
**Action:** Always replace verbose `!== undefined && !== null` checks and complex conditional fallbacks with loose inequality (`!= null`) and the nullish coalescing operator (`??`) to simplify conditional execution paths and reduce cognitive load.
