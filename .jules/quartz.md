...
## 2026-05-27 - Pass union-typed variables directly
**Learning:** When variables are strictly typed to a specific union (e.g., `'NeprofileUser' | 'NeaccessUser'` via `getRoleType`), unwrapping them through redundant `if-else` blocks that check for one value and fall back to another creates unnecessary code branching.
**Action:** Pass the union-typed variable directly to consuming functions rather than expanding it into a conditional structure.
