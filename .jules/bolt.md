...
## 2024-07-28 - Optimize array flat allocations
**Learning:** Wrapping values in an array and flattening them (e.g. `[value].flat().map(...)`) purely to homogenize iteration generates significant GC pressure during massive nested profile resolution, as both `.flat()` and the outer `[]` array produce short-lived heap allocations.
**Action:** To minimize array allocation overhead in hot paths, avoid using `[value].flat().map(...)`. Instead, use an allocation-free ternary with an explicit type check, such as `Array.isArray(value) ? value.map(...) : [value.property]`.
