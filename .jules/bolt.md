...
## 2024-07-24 - Avoid allocating intermediary arrays via flat()

**Learning:** To minimize array allocation overhead and garbage collection pressure when handling variables that could be either a single object or an array, avoid using `[value].flat().map(...)`. Using `.flat()` creates intermediate array allocations.
**Action:** Use an allocation-free ternary with an explicit type check, such as `Array.isArray(value) ? value.map(...) : [value.property]` or `Array.isArray(value) ? value : [value]`.
