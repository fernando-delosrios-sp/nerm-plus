...
## 2026-06-14 - Optimize path traversal allocation
**Learning:** Using array-based tokenization like `.split('.').reduce()` for path manipulation adds O(N) allocation overhead in hot loops and can introduce logical bugs.
**Action:** Always favor O(1) string slicing using `indexOf('.')` and `slice()` for deep object paths in highly iterative sections.
