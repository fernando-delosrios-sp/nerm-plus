...
## 2024-05-24 - Optimize Array Allocation overhead for single vs array types
**Learning:** Using `[value].flat().map(...)` on variables that can be objects or arrays causes unnecessary array allocation and garbage collection pressure in hot paths.
**Action:** Use an allocation-free ternary with an explicit type check, like `Array.isArray(value) ? value.map(x => x.prop) : [value.prop]`.
