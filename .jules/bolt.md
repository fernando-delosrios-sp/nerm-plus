...
## 2024-08-26 - Cache ISCClient Configuration Retrieval

**Learning:** `listSources` and `listSourceSchemas` on `ISCClient` were issuing un-cached network requests to retrieve static source integration configurations. When processing multiple accounts or entities concurrently, this resulted in severe N+1 API fetching performance bottlenecks.
**Action:** Implemented a promise-based caching pattern on the `ISCClient` class to memoize the static source mappings and schemas per connection session.
