...
## 2024-07-23 - Optimize Array to Set Lookups
**Learning:** Using `Array.includes()` for checking static lists of configuration attributes (e.g. `USERONLY_ATTRIBUTES`) creates O(N) overhead during hot path loops like attribute extraction and mapping.
**Action:** When performing membership testing in hot loops, define static configuration lists as `Set` objects and use `Set.prototype.has()` to reduce time complexity to O(1).
