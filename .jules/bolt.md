...
## 2024-05-24 - Replace inefficient array creation and flattening in mapping loops
**Learning:** Using `[value].flat().map(...)` inside hot loops causes severe O(N) allocation overhead and garbage collection pressure because it creates a new intermediate array instance on every iteration before mapping.
**Action:** Replace `[value].flat().map(...)` with an allocation-free ternary `Array.isArray(value) ? value.map(...) : [value.property]`. This skips unnecessary array allocations.
