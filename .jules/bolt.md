...
## 2024-05-20 - String Slicing over Array Allocation
**Learning:** Using array-based tokenization like `.split('.').pop()!` or `.split('.').reduce()` for path manipulation adds O(N) allocation overhead in hot loops and can introduce logical bugs.
**Action:** Always favor O(1) string slicing using `lastIndexOf('.')`, `indexOf('.')`, and `slice()` for deep object paths.
