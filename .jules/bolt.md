...
## 2025-03-02 - Batched Asynchronous Promise Resolution for N+1 Query Patterns
**Learning:** When pushing large amounts of independent, non-throttled async API calls (such as deletions) onto an array for \`Promise.all()\` inside a \`for await\` loop, we run into N+1 issues and memory bottlenecks. The system loads every single promise into memory simultaneously, exceeding concurrency limits and causing massive heap consumption for large datasets.
**Action:** Always batch array promise accumulations directly within the event loop iterator using an accumulator threshold (e.g., \`BATCH_SIZE\`). Await the batch with \`Promise.all(batch)\`, process errors, and flush the array before proceeding to the next chunk. Remember to evaluate the remaining batch array outside the loop body for any trailing data.
