...
## 2024-06-25 - Parallelize Entitlement Additions
**Learning:** Sequential await calls in for loops can cause major performance bottlenecks for independent operations (e.g. adding roles or types).
**Action:** Always map independent async calls to an array of promises and await them concurrently using `Promise.all()` to decrease overall execution time.
