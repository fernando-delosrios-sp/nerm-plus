...
## 2024-02-12 - Optimize Array Aggregation loops and Map Lookups
**Learning:** Using array spread operator `...` inside `.push()` within an iterative loop causes O(N) internal allocations leading to O(N²) worst-case behavior or stack overflow exceptions. Furthermore, using `.has()` to check existence before `.set()` and `.get()` doubles the Map lookup overhead.
**Action:** Use native `Array.prototype.flatMap()` for safe and highly optimized array mapping and collection instead of manual spread pushes in loops. Consolidate map lookups by capturing the `.get()` result into a local variable and initializing if absent.
