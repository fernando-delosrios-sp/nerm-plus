...
## 2024-05-31 - Optimize mapping loop allocations in entity2profile
**Learning:** Using `Object.entries(obj).forEach(([key, val]) => ...)` in hot paths like `entity2profile` creates significant heap allocation overhead (arrays and closures).
**Action:** Replace `Object.entries(...).forEach` with a simple `for...in` loop to avoid intermediate array allocations and improve throughput.
