...
## 2024-05-19 - Concurrent I/O execution for sequential updates
**Learning:** Sequential processing in deep iteration logic (like `for (const change of changes) { for (const value of values) { await apiCall(...) } }`) creates massive N*M bottlenecks. In `src/operations/std-account-update.ts`, this was taking ~5300ms for batches.
**Action:** Use `Promise.all` mapping to aggregate promises, but handle dependency chains (e.g. reload account properties only after 'types' complete but before 'roles' run). By grouping independent tasks into grouped `typePromises` and `otherPromises` arrays and sequentially executing the `Promise.all` batches, we maintained strict ordering requirements while enabling concurrency. Batch time reduced from ~5300ms to ~60ms.
