...
## 2024-07-31 - Optimize single-object array processing overhead

**Learning:** When handling variables that could be either a single object or an array of objects, wrapping the variable in an array, calling `.flat()`, and then `.map()` (e.g., `[value].flat().map(x => x.id)`) creates unnecessary intermediate heap allocations and garbage collection pressure in hot loops.
**Action:** Minimize array allocation overhead by using an allocation-free ternary with an explicit type check instead, such as `Array.isArray(value) ? value.map(x => x.id) : [value.id]`.
