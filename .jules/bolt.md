...
## 2024-05-31 - Remove [value].flat() in hot loops
**Learning:** Using `[value].flat().map()` to handle values that may be single objects or arrays causes unnecessary heap array allocation overhead and garbage collection pressure in highly-iterative hot paths (e.g. `resolveProfileAttributes`).
**Action:** When handling a variable that could be an array or a single object, avoid `[value].flat().map(...)`. Use an allocation-free ternary with an explicit type check instead, such as `Array.isArray(value) ? value.map(...) : [value.property]`.
