...
## 2025-02-12 - Removed array flat map allocation bottleneck
**Learning:** The `[value].flat().map(...)` pattern in hot loops (like attribute extraction) introduces significant array allocation and garbage collection overhead, particularly when run inside highly concurrent Promise.all maps.
