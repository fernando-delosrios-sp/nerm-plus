...
## 2024-08-25 - Avoid unnecessary array flattening in hot paths
**Learning:** Using `[value].flat().map(...)` inside a tight loop or frequently called function (like attribute resolution) introduces unnecessary array allocation overhead and garbage collection pressure, especially when the `value` is rarely a nested array.
**Action:** Replace `[value].flat().map(...)` with an allocation-free ternary like `isArray ? value.map(...) : [value.property]`, reusing the already-calculated `isArray` boolean to skip unnecessary wrapper array creation and flattening.
