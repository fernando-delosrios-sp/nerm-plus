...
## 2025-01-20 - Concurrent Async Generator Consumption
**Learning:** Sequential processing blocks stream consumption in `for await` loops, creating a performance bottleneck when network IO is involved. However, naively collecting all promises without bounds leads to unbounded concurrency and memory exhaustion.
**Action:** When replacing sequential `await` processing with concurrent batching, implement a bounded execution pool (e.g., using a `Set<Promise>`) that tracks in-flight operations and enforces backpressure (e.g., `await Promise.race(executing)` when size hits max). Ensure all promises are `.finally()` cleaned up, and explicitly track `.catch()` errors in a separate array to re-throw at synchronization points, preventing both Unhandled Rejection crashes and silently swallowed errors.
