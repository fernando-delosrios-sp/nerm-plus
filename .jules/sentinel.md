...
## 2026-06-25 - Prevent secrets leaking from bypassed central loggers
**Vulnerability:** In `nerm-client.ts`, certain low-level methods (`listRequest`, `getRequest`, `deleteRequest`) were manually constructing log strings from HTTP errors (e.g., `this.logError('listRequest', e.response?.data?.error ?? e.message ?? \`\${e}\`)`) instead of passing the full error to the centralized `formatHttpError` utility. Furthermore, `formatHttpError` itself was failing to sanitize simple string HTTP responses, returning them directly (`${prefix}: ${data}`).
**Learning:** Even if a robust, centralized error redactor exists (`toLogString`), it is useless if methods bypass it by performing their own ad-hoc error stringification, or if the redactor ignores primitive payload strings (which might be URL-encoded data or stringified JSON).
**Prevention:** Ensure that every network request method uniformly passes raw caught errors through the centralized formatter (like `formatHttpError`). Also, ensure the formatter explicitly passes plain string response bodies through the redaction layer (`toLogString`) rather than blindly assuming strings are safe.
