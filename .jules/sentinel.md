...
## 2024-07-11 - Prevented Credential Leakage in Formatted Errors
**Vulnerability:** Sensitive API data was concatenated directly into error message strings before being logged, bypassing object/JSON redaction mechanisms.
**Learning:** String interpolation bypasses redaction utilities that expect structured data. If a raw string is interpolated into another string (e.g., `HTTP 400: ${data}`), it creates a complex string that might fail redaction parsing, leaking sensitive data in plain text.
**Prevention:** Always pass variables through redaction utilities (like `toLogString(data)`) *before* interpolating them into log messages or formatted error strings.
