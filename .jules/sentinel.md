...
## 2024-06-18 - Prevent Path Traversal in NERMClient
**Vulnerability:** Unsanitized dynamic IDs used in API request paths (`/profiles/${id}`).
**Learning:** Hardcoded template literal paths lack encoding, exposing the client to path traversal or URL injection if an `id` contains slashes (`/`) or special characters.
**Prevention:** Always sanitize dynamic path parameters by explicitly casting to string and wrapping them in `encodeURIComponent(String(param))` to safely handle non-string or nullish inputs.
