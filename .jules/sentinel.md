...
## 2024-06-14 - Prevent path traversal and URL injection in API requests
**Vulnerability:** Dynamic path parameters used in API URL template strings (e.g., `/users/${id}`) were not being explicitly URL-encoded, allowing potential path traversal (`../`) or URL injection vulnerabilities if malicious inputs were processed.
**Learning:** Utilizing template strings for URL paths safely requires manual URL encoding of the variables, as basic string interpolation does not provide security guarantees against special characters or path segment separators.
**Prevention:** Always sanitize dynamic path parameters by explicitly casting to string and wrapping them in `encodeURIComponent(String(param))` when constructing API request paths.
