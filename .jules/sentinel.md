...
## 2026-05-13 - Prevent Path Traversal URL Injection
**Vulnerability:** Path traversal and URL injection vulnerabilities in src/nerm-client.ts where dynamic parameters (like IDs) were passed to template string API paths without escaping.
**Learning:** Raw dynamic values passed to string templates can be manipulated to exploit path traversal if they contain special characters like .. or /.
**Prevention:** Always wrap dynamic path variables in API request strings with encodeURIComponent() to ensure characters cannot alter the path hierarchy.
