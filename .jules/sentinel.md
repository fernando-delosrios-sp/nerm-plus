...
## 2024-10-24 - Fix Exposed sensitive data in error messages
**Vulnerability:** HTTP Error messages logged plain text strings (like `data` or `data.error`) without redacting sensitive data.
**Learning:** String interpolation bypassed the existing redaction utility `toLogString` which is designed to identify and mask strings containing secrets (via URLs or stringified JSON).
**Prevention:** Always pass untrusted or external strings through the redaction utility (`toLogString(data)`) prior to string interpolation into logs or errors.
