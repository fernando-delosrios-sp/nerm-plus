...
## 2026-06-25 - Prevent Exposing Sensitive Query Parameters in Retried Requests

**Vulnerability:** The logger in `src/axios.ts` directly interpolated `requestConfig.url` into a string when retrying a request via `axios-retry`. Since the original URL might contain sensitive query parameters (like `password`, `token`, `secret`, `api_key`), these parameters were logged in plain text.
**Learning:** Even when logging request configurations or metadata (like retries), any direct inclusion of original URLs or request data can bypass centralized logging redaction mechanisms if not explicitly passed through them.
**Prevention:** Always pass interpolated request data, particularly URLs and request configurations, through a central redaction utility (e.g., `toLogString`) before logging them in plain text, especially in custom hooks or retry logic.
