...
## 2024-05-17 - Sensitive data leak in HTTP error logs
**Vulnerability:** HTTP errors in list, get, and delete methods were formatting the error message manually (e.g. `e.response?.data?.error`), bypassing the centralized `formatHttpError` which implements `toLogString` to safely redact secrets.
**Learning:** Inconsistent ad-hoc error stringification inside client requests risks exposing credentials or sensitive payload data that might be embedded in raw upstream HTTP responses.
**Prevention:** All HTTP responses should be passed through a centralized error formatter that redacts sensitive attributes before being written to logs.
