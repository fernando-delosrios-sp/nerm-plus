...
## 2024-05-18 - Unredacted string interpolation in error formatting
**Vulnerability:** API error responses formatted in `formatHttpError` interpolated string responses and string errors directly into the returned error message. If the API returns stringified JSON (e.g., validation errors containing secrets) or plain text secrets, they are leaked into logs and connector error messages, bypassing the `toLogString` redaction utility.
**Learning:** When string values from external sources (like HTTP response `data` or `data.error`) are interpolated directly into template literals, they bypass object-based redaction mechanisms entirely. This can easily happen in catch-all error formatters.
**Prevention:** Always wrap raw string inputs from external API responses in a redaction utility (like `toLogString()`) before interpolating them into log statements or returned error strings.
