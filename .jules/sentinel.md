...
## 2024-06-12 - Sanitize Dynamic Path Parameters in API Requests
**Vulnerability:** Dynamic path parameters (e.g., IDs) in `NERMClient` were interpolated directly into URL paths without sanitization, leading to potential path traversal and URL injection vulnerabilities.
**Learning:** Even when expected to be simple UUIDs or alphanumeric IDs, unsanitized inputs in URL paths can be exploited or cause malformed requests if edge cases or malicious inputs are provided.
**Prevention:** Always sanitize dynamic path parameters by explicitly casting to string and wrapping them in `encodeURIComponent(String(param))` to safely handle non-string or nullish inputs, and prevent URL injection.
