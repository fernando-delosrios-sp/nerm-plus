...
## 2026-05-13 - Error Serialization Bypassing Redaction in Request Catch Blocks

**Vulnerability:** HTTP error payloads in `nerm-client.ts` (`listRequest`, `getRequest`, and `deleteRequest`) were logged using raw interpolation (`${error}` or `e.response?.data?.error`), bypassing the `formatHttpError` function which correctly leverages `toLogString` to redact sensitive API payload fields.
**Learning:** Hardcoded `.catch` blocks on low-level HTTP requests are prime locations for missing standard error formatting, especially if methods were copied/pasted or refactored independently. These easily leak unredacted stringified JSON error payloads that could contain API keys, passwords, or bearer tokens returned by the server on failures.
**Prevention:** Ensure all network error logs pass through a standardized HTTP error formatter (like `formatHttpError`) that internally routes the full response data structure through the application's global log redaction utility before writing to the log stream.
