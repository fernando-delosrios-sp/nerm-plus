...
## 2026-06-08 - [Prevent Path Traversal in API Client]
**Vulnerability:** Path traversal and URL injection via unsanitized path parameters in API request URLs.
**Learning:** Dynamic path parameters (e.g., IDs) were directly interpolated into URL strings without sanitization. Even if expected to be UUIDs, lack of encoding could allow malicious input to traverse the API path hierarchy.
**Prevention:** Always wrap dynamic path parameters in encodeURIComponent() before interpolating them into URL strings.
