...
## 2026-05-18 - Path Traversal Prevention in API Clients
**Vulnerability:** Unsanitized dynamic variables (e.g., `id`, `profile_id`) were directly interpolated into REST API paths in `NERMClient`.
**Learning:** This exposes the application to path traversal or URL injection if an attacker inputs values containing special characters like `../` to access unintended resources.
**Prevention:** To prevent URL injection vulnerabilities, always sanitize dynamic path parameters by wrapping them in `encodeURIComponent(String(param))` to safely handle inputs and neutralize traversal strings.
