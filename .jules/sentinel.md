...
## 2025-02-13 - Prevent Sensitive Data Leak in String Interpolation
**Vulnerability:** Raw attribute values (like passwords) were interpolated directly into log templates (`logger.debug(\`Setting attribute ${attribute} to value ${value}\`)`), bypassing the JSON-based redaction utility (`toLogString`).
**Learning:** When using logging redaction utilities designed for JSON or object graphs, interpolating sensitive primitive strings directly into template literals prevents the redaction logic from identifying the sensitive key context.
**Prevention:** Always use explicit redaction checks (like `isSensitiveKey(attribute) ? '[REDACTED]' : value`) when logging string-interpolated primitives, or structure the logged data as objects and pass them entirely through the redaction utility.
