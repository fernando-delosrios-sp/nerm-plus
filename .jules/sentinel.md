...
## 2026-07-27 - Exposed sensitive headers in AxiosError logs
**Vulnerability:** Raw HTTP `_header` and `_pendingData` string properties in AxiosError objects were bypassing the redacting logic when stringified into error logs. This leaked unredacted `Authorization: Bearer` tokens and secrets hidden inside URL query paths on `ECONNREFUSED` or `429` rate limit retries.
**Learning:** `JSON.stringify` on deeply nested error objects serialized raw HTTP payloads containing sensitive strings. The custom redact function failed to intercept or clean them because it didn't inspect large strings for embedded secrets.
**Prevention:** Always intercept known raw-request string fields (`_header`, `_pendingData`) during object redacting and perform Regex-based redaction on known sensitive headers and paths inside them before serialization.
