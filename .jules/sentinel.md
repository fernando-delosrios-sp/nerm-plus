...
## 2024-06-15 - Prevent URL Injection/Path Traversal with encodeURIComponent
**Vulnerability:** Un-sanitized string interpolation in API request paths
**Learning:** Missing `encodeURIComponent` could allow unexpected inputs (like `../` or spaces) to traverse out of the expected endpoint.
**Prevention:** Always use `encodeURIComponent` when embedding dynamic variables into URLs.
