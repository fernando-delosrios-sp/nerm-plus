...
## 2024-06-13 - Path Traversal Vulnerability in API Clients
**Vulnerability:** Path traversal vulnerabilities (and potential URL injection) were present in the NERM client because raw dynamic parameters (e.g., `id`, `name`) were interpolated directly into request URLs (e.g., `url = \`/profiles/\${id}\``) without sanitization.
**Learning:** Even when inputs are expected to be numeric IDs or simple strings, failing to encode parameters opens the door to URL manipulation (e.g., injecting `../` or query parameters like `?isAdmin=true`).
**Prevention:** Always wrap dynamic URL path parameters with `encodeURIComponent(String(param))` to safely escape structural characters and enforce string casting.
