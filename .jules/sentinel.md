...
## 2026-07-09 - Prevent Sensitive Credential Leakage in String Interpolation
**Vulnerability:** Error messages in `src/nerm-client.ts` were constructed by interpolating arbitrary dynamic variables (like `err.message` or `data` from HTTP responses) directly into string templates before redaction, or by forcibly stringifying raw Error objects (e.g., `` `${error}` ``). This bypasses the redaction logic in `toLogString` (which only parses JSON objects when strings resemble JSON), potentially leaking plain-text passwords or sensitive URLs in logs when API requests fail.
**Learning:** The built-in JavaScript template literal evaluation (`${}`) blindly converts complex objects to generic strings (like `[object Object]` or `Error: ...`), obscuring their internal structure before custom redaction logic can process them.
**Prevention:** Always pass dynamic error variables and raw objects through redaction utilities (like `toLogString`) *before* interpolating them into string templates, or pass them natively as raw objects to logging utilities so redaction can traverse their properties correctly.
