...
## 2024-08-11 - Prevent Sensitive Data Leak in Stringified Error Interpolation
**Vulnerability:** Stringified HTTP error payloads like `data.error` or `data.message` were being directly interpolated or pushed into log arrays without being passed through `toLogString()`, bypassing redaction heuristics for JSON and URL parameters.
**Learning:** Directly interpolating error payloads (e.g., `${prefix}: ${data}`) changes the string format before it hits the redaction utility (if it hits it at all), which breaks the utility's ability to identify and parse valid JSON or URL structures, leading to unredacted sensitive data in logs.
**Prevention:** Always pass the raw string payload or error message explicitly through the redaction utility (e.g., `toLogString(data)`) *before* interpolating it into a larger log string or pushing it to a collection.
