...
## 2024-07-04 - Prevent String Interpolation Bypassing Redaction in Error Messages

**Vulnerability:** In `formatHttpError`, raw HTTP response data and error properties (when they were strings) were directly concatenated into error strings (e.g., `` `${prefix}: ${data}` ``) before being logged out or bubbled up. This bypassed `toLogString()`, causing stringified JSON payloads containing secrets (like `{"password": "secret"}`) to be logged in plain text.
**Learning:** Checking `typeof x === 'string'` and assuming a string is safe to log raw is a dangerous pattern. APIs frequently return full JSON objects encoded as strings inside error payload fields, which may contain sensitive credentials.
**Prevention:** Always pass strings through the log redaction utility (`toLogString`) rather than directly interpolating them into log messages or error wrappers, since robust redactors can detect and parse stringified JSON.
