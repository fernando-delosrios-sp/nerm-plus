...
## 2025-01-20 - Fix sensitive data leakage in retry logs
**Vulnerability:** URLs containing sensitive query parameters (like passwords or secrets) were being logged in plain text when API requests were retried.
**Learning:** Directly interpolating dynamic strings (like URLs or JSON payloads) into template literals bypasses redaction utilities.
**Prevention:** Always wrap variables representing full URLs or stringified payloads in `toLogString()` before interpolating them into log messages.
