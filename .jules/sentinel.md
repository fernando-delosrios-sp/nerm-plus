...
## 2026-06-23 - Prevent secrets leaking in HTTP error string attributes
**Vulnerability:** HTTP error messages or attributes (`data.error` or `data.message`) that were of type `string` were being pushed directly to log message parts without redaction. They bypassed the central `toLogString` filter.
**Learning:** String properties returned in HTTP errors often contain stringified JSON objects or raw URLs that embed credentials or sensitive payload structures.
**Prevention:** Treat any external, untrusted error data strings as potentially sensitive and always pass them through the standard redaction utility (e.g. `toLogString`) rather than interpolating or logging them directly.
