...
## 2025-05-25 - Prevent secrets leak via URL-encoded Axios payloads in logs
**Vulnerability:** The logger successfully redacted JSON payloads containing secrets, but failed to intercept and redact secrets formatted as URL-encoded strings (e.g. `client_secret=...&client_id=...`), which can be generated by Axios when form-data or standard web payloads fail and are embedded inside AxiosError payloads.
**Learning:** Security controls (like PII/secret scrubbing) often fail to account for variations in data serialization formats, leading to blindspots if input isn't structured as expected (like stringified JSON).
**Prevention:** Implement defense-in-depth in scrubbing utilities by scanning generic strings for fallback formatting (like `x-www-form-urlencoded` using `URLSearchParams`) in addition to standard JSON parsing. Always test redaction logic with strings simulating HTTP payload bodies in error scenarios.
