...
## 2026-05-13 - Prevent Path Traversal in API Clients
**Vulnerability:** Dynamic path parameters (e.g., user IDs) were directly interpolated into URL strings in `nerm-client.ts` without sanitization. If a user provided an ID like `../../admin`, it could lead to path traversal or Server-Side Request Forgery (SSRF) when making requests to the underlying API.
**Learning:** Even internal or backend-to-backend API clients need robust input sanitization, as variables passed down from upstream inputs might not be completely sanitized. Unescaped path parameters can easily break API routing.
**Prevention:** Always wrap dynamically injected path variables with `encodeURIComponent()` when constructing API request URLs to ensure special characters (like slashes or query parameter delimiters) are properly escaped and the URL structure remains intact.
