...
## 2026-06-06 - Prevent Path Traversal and URL Injection
**Vulnerability:** Unsanitized dynamic IDs (e.g. `profile_id`, `user_id`) were interpolated directly into Axios URL strings, allowing path traversal and injection attacks.
**Learning:** Path parameters provided dynamically must always be URL encoded, otherwise special characters like `../` or `?` can change the intended API endpoint.
**Prevention:** Always wrap dynamic path segments with `encodeURIComponent()` when constructing API request paths.
