...
## 2024-05-28 - Prevent Path Traversal in API Request Paths
**Vulnerability:** Dynamic path parameters in API request paths (e.g., `id`, `profile_id`) were interpolated into URLs without sanitization.
**Learning:** Constructing API endpoint paths using raw user or entity input can expose the application to path traversal or URL injection attacks if the IDs contain `/` or other special URL characters.
**Prevention:** Always wrap dynamic path variables in `encodeURIComponent()` when constructing API request paths.
