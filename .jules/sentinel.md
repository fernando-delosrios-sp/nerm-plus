...
## 2025-02-28 - Fix redaction bypass in HTTP error formatter
**Vulnerability:** Stringified HTTP error payloads (like `data.error` or `data.message`) were directly interpolated into error messages without being explicitly passed through the `toLogString()` function.
**Learning:** The redaction utility `toLogString()` handles URLs and JSON objects, but if a sensitive token or password is leaked as part of a raw string inside an HTTP error response, it bypasses redaction unless the entire string structure is parsed or formatted through `toLogString()`. By skipping the explicit wrapper for raw error strings or objects, the fallback interpolation leaks unredacted secrets into the application logs.
**Prevention:** Ensure that all data sources (including strings and inner object fields like `.error` and `.message`) that might contain untrusted or sensitive response data are passed directly through `toLogString()` before being concatenated or interpolated into a logged message.
