...
## 2026-08-18 - Stringified JSON Error Responses Bypass Redaction
**Vulnerability:** HTTP error payloads in `src/nerm-client.ts` were checked with `typeof data === 'string'`. If true, the raw string was returned without redaction (`${prefix}: ${data}`). This allows error responses containing stringified JSON (like failed auth payloads containing secrets or tokens) to be logged in plain text.
**Learning:** Checking for `typeof data === 'string'` is often a trap. While primitive strings may not be sensitive, API error payloads are frequently serialized JSON strings. Redaction libraries that parse JSON internally (like the custom `toLogString` in this repo) must still be applied to these strings.
**Prevention:** Never skip security stringification or redaction functions on raw string responses. Always wrap error payload strings in the established redaction utility (`toLogString(data)`) to ensure embedded JSON secrets are caught and sanitized.
